...
namespace ConsoleApp8
{
    class Menu //Основное меню приложения
    {
        ...
        public void Menu1(string[] options, string prompt1)
        {
            Prompt1 = prompt1;
            Options1 = options;
...
Добро пожаловать в систему!!!";
            ...
            m:
                ...
                ResetColor();
                string a, login;//присваиваю значение логинов
                a = ReadLine();
                string rep = @"C:\Users\Пользователь\source\repos\Metadon1.1\Metadon1.1\bin\Debug\работники.txt";

                using (StreamReader ui = new StreamReader(rep))
                {
                    Console.WriteLine(ui.ReadToEnd());
            ...
            void NewUser1()
            {
                ...
                WriteLine(" \n<<Если хотите добавить сотрудника нажмите Enter>> ");
                ResetColor();
                ConsoleKey keyPressed;
                ConsoleKeyInfo keyInfo = ReadKey(true);
                keyPressed = keyInfo.Key;
            ...
            h:
                ...
                p:
                    ForegroundColor = ConsoleColor.Green;
                    WriteLine("<< пароль(три цифры,три буквы и три спец символа):>>");
                    ResetColor();
                    Passwd = ReadLine();
                    ...
                    foreach (char i in Passwd.Where(char.IsUpper))
                        z = z + 1;
                    if (Passwd.Length >= 8 && y > 0 && x > 0 && l > 0 && w > 0 && c > 0 && z > 0 && m.Success)
                    {

                    g:
                        var rep = new StreamWriter(@"C:\Users\Пользователь\source\repos\Metadon1.1\Metadon1.1\bin\Debug\работники.txt", true);//система сохранения новых пользователей
                        ForegroundColor = ConsoleColor.Green;
                        WriteLine("<<Введите пароль снова:>>");
                        ResetColor();// закрыть файл для записи
                        a = ReadLine();
